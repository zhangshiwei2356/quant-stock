...
| 二级菜单 | 功能 |
|----------|------|
| 回测工作台 | **仅从目标池**选股（下拉全量直选 + 池内搜索不截断）；参数分「标的 / 时间资金 / 策略」三组；主按钮执行回测 → K 线信号 + 权益 |
...
| 二级菜单 | 功能 |
|----------|------|
| 回测工作台 | **仅从目标池**多选（批量选择：全选/清空/反选；已选标签紧贴选股）；时间/资金/策略同行；「执行组合回测」品牌蓝主按钮；共享资金池、强制日 K |
| 回测历史 | 组合历史与分析 |
...
| 二级菜单 | 功能 |
|----------|------|
| 当前池 | 扫描漏斗（无数据时引导扫描）；「扫描更新」主按钮 /「刷新列表」次按钮 /「下载报告」文字链接；入池原因单行截断悬停查看；移出悬停变红并二次确认（**移出≠卖出**） |
| 扫描历史 | 批次与报告；亦可手动扫描 |
...
资金权益（通栏总权益/可用现金 + 盈亏/风控/状态三列；金额右对齐；恢复=绿主按钮、手动退役=次按钮、强制恢复=橙警告）· 持仓（批次 T+1）· 委托 · 权益日结 · 风控事件。  
侧栏数量角标为 0 时隐藏。数据来自本地模拟账本表（非真实柜台）。
...
侧栏「扩展与文档」一级菜单（工作台之后、量化知识之前）：点测 OES/MDS 运维接口（入参/出参 JSON）+ 二级「宽睿文档梳理」（**工作台内嵌**，不切文档模式）。

| 二级 | 功能 |
|------|------|
| 接入总览 | **接入结论条**（可报撤 / 可联调只读 / 有账号未 live 等）+ 账号凭据 / MDS / OES / 报撤 / 静态状态卡；跳转以卡片「进入」为准 |
| 账号登录 | 进入自动展示当前生效账号 → `GET /api/ops/kuangrui/account/current`（无密码）；登录先验柜再密文入库；取密 **DB active 优先**，否则 env；主密钥表 `kuangrui_crypto_key`（两表不进数据表白名单） |
| OES 只读 | 左窄列表（「介绍」弹层摘要 +「调用」；可选入参默认收起）；右侧入参/出参 + 空态引导；资金/持仓/委托/成交/快照/对账/证券/交易日/佣金/总览/股东账户/主柜资金/可买量/stop |
| MDS 行情 | 同上布局；状态/静态/证券状态/时段/合并静态；pull·subscribe·flush·stop（写操作二次确认） |
| 报撤试单 | 报单 / 撤单 / 银证 **分 Tab**；报单与银证为橙色高危按钮；闸状态条；须 `orderLive` + 二次确认；银证高级参数默认可折叠 |

默认旁路关闭；不改金叉主路径。对接手册见「宽睿对接 → 宽睿文档梳理」；联调页「介绍」可跳转该文档。

### 10. 量化知识 / 应用说明

- **量化知识**：侧栏按 **基础 / 指标 / 机制 / 评价** 分组；阅读区支持上一篇/下一篇（键盘 ← →）、文内目录（长文）、交叉引用可点、「本系统用法」高亮、文末相关功能入口；介绍页可「从 A股基础开始」或下载 PDF。涵盖 A 股基础、指标、涨跌停、T+1、成本、仓位、风控、撮合、**夏普/回撤/胜率与综合评分口径**、回测要点等
- **应用说明**：侧栏按 **入门 / 规则 / 路线** 分组；阅读区四篇快切 + 翻篇（← →）+ 长文/README 目录 + 相关入口（可跳宽睿）；含系统概述 → **项目 README** → 交易规则 → 能力与待办；介绍页可「从系统概述开始」或下载 PDF
- **宽睿对接**：联调点测 + **宽睿文档梳理**
- 介绍页 / 阅读顶栏可导出 PDF：`GET /api/docs/pdf/{stock|app}`
...
- 主操作按钮统一品牌蓝（含「执行组合回测」）；`danger` 仅用于高危操作；表格统一表头/斑马纹/行 hover，`code`/技术标识弱化为小号浅灰
- 工作台顺序：**工作台** → **行情** → **个股回测** → **组合回测** → **目标池** → **账户** → **运维中心** → **策略管理** → **数据表**
