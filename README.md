...
- 进入应用先显示**首页仪表盘**（`docs/home.html`）：账户摘要 / 策略 / 数据健康四卡 + 分组功能入口 + 最近访问
- 侧栏一级菜单**仅展开/收起**二级；点击二级（或入口卡）直达功能；顶栏 Logo 回仪表盘
- 模块介绍（`docs/nav-*.html`）改为顶栏「帮助」可选阅读，不再作为进功能的必经路径
- 侧栏为横向分组（工作台 / 扩展与文档）+ SVG 图标；可收起为纯图标窄栏（`localStorage`）；主区有面包屑；菜单展开与最近工作区可记忆恢复
