...
| `quant.kuangrui.oes.enabled` / `oes.order-enabled` | OES 只读对账 / 报单总闸（**默认 false**）；M2 只开 `oes.enabled`；报撤见 M3 |
...
| GET/POST `/api/ops/kuangrui/oes/*` | 宽睿 OES 只读：status/cash/holdings/orders/trades/snapshot/reconcile/stop（默认 noop；`-Pkuangrui`+开关） |
...
- 已实现：`scan-and-trade` / `pool-rebuild` / `after-market-batch-scan` / `settle-after-close` / `data-validate` / `factor-daily-rebuild` / `day-collect` / `pool-minute-backfill` / `sync-orders` / `position-pnl-sync`
  ...
  - `sync-orders`：本地桩将 `SUBMITTED→FILLED` 并改仓；`trade-mode=sdk` 时策略在 sync 后才落现金/批次；OES live 时额外对账日志（不改仓）
  - `position-pnl-sync`：本地成本 + 最新价浮盈日志；OES live 时附加柜台资金/持仓对账
- 页面标「未实现/缺外部默认」：`market-collect`（本地骨架；**可选**宽睿 MDS live 时 pull/flush，见下）
- 对照：**应用说明 → 能力与待办**；宽睿对接：**应用说明 → 宽睿文档梳理**（M0✓ → M1 MDS 可选✓ → M2 OES 只读可选✓ → 报撤 → 静态/费率）
...
- 宽睿 **M2**（可选，默认关）：`quant.kuangrui.enabled/oes.enabled=true`（`oes.order-enabled` 仍关）；登录+`sendRptSync`+查资金/持仓/委托/成交；运维 `GET/POST /api/ops/kuangrui/oes/{status,cash,holdings,orders,trades,snapshot,reconcile,stop}`；纸面对账可见柜台数据；不下单
