 ...
 * M4：当宽睿 {@code static-enabled} 时，对「今天」优先参考 OES {@code queryTradingDay}，失败回退本地。
 * </p>
 */
@Component
public class TradingCalendar {
    ...
    /** Spring 注入入口。 */
    @Autowired
    public TradingCalendar(ObjectProvider<KuangruiStaticInfoService> staticInfoProvider) {
    ...
    public boolean isAshareTradingDay(LocalDate date) {
        ...
        if (date.equals(today)) {
            ...
            if (s != null && s.isApplyEnabled()) {
                LocalDate td = s.exchangeTradingDay();
                if (td != null) {
                    return td.equals(date);
                }
            }
        }
