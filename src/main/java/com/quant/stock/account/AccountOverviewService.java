...
public class AccountOverviewService {
    ...
    public Map<String, Object> summary() {
        ...
        m.put("hint", "本地模拟账本（非券商柜台）。成交/持仓/现金已落库，重启可恢复；券商对账见「能力与待办」。");
        ...
        m.put("haltReason", accountRiskState.getHaltReason());
        m.put("underwaterTradingDays", accountRiskState.getUnderwaterTradingDays());
        m.put("drawdownDurationReduceDays", quantProperties.getDrawdownDurationReduceDays());
        m.put("drawdownDurationHaltDays", quantProperties.getDrawdownDurationHaltDays());
        m.put("consecutiveLosses", accountRiskState.getConsecutiveLosses());
        m.put("positionScale", accountRiskState.positionScale(equity));
        boolean allowRisk = accountRiskState.allowNewOpen(LocalDate.now(), equity);
        boolean allowRetire = strategyRetirementService.allowNewOpen();
        boolean allowData = !dataReconcileGateService.blockNewOpen();
        m.put("allowNewOpen", allowRisk && allowRetire && allowData);
        m.put("dataReconcileBlock", !allowData);
        m.put("strategyRetired", strategyRetirementService.isRetired());
        m.put("retirement", strategyRetirementService.status(LocalDate.now()));
