...
public final class LimitBoardHelper {
    ...
    public static BigDecimal limitUpPrice(BigDecimal prevClose, String stockCode) {
        ...
        return prevClose.multiply(BigDecimal.ONE.add(limitPct(stockCode)))
    ...
    public static BigDecimal limitDownPrice(BigDecimal prevClose, String stockCode) {
        ...
        return prevClose.multiply(BigDecimal.ONE.subtract(limitPct(stockCode)))
                .setScale(2, RoundingMode.HALF_UP);
    }
    ...
    public static boolean isLimitUp(BarDTO cur, BigDecimal prevClose, String stockCode) {
        return isLimitSide(cur, prevClose, stockCode, true);
    }

    public static boolean isLimitDown(BarDTO cur, BigDecimal prevClose, String stockCode) {
        return isLimitSide(cur, prevClose, stockCode, false);
    }

    private static boolean isLimitSide(BarDTO cur, BigDecimal prevClose, String stockCode, boolean up) {
        if (cur == null || prevClose == null || prevClose.compareTo(BigDecimal.ZERO) <= 0) {
            return false;
        }
        String code = stockCode != null ? stockCode : cur.getCode();
        BigDecimal limitPx = up ? limitUpPrice(prevClose, code) : limitDownPrice(prevClose, code);
        if (limitPx == null) {
            return false;
        }
        ...
        if (close != null) {
            BigDecimal soft = prevClose.multiply(up
                    ? BigDecimal.ONE.add(limitPct(code).multiply(new BigDecimal("0.95")))
                    : BigDecimal.ONE.subtract(limitPct(code).multiply(new BigDecimal("0.95"))))
