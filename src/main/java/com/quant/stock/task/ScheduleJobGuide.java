...
public final class ScheduleJobGuide {
    ...
    static {
        ...
        m.put("scan-and-trade", new Detail(
                ...
                "默认工作日 9–11、13–15 点每分钟一次（CRON）。",
                "可能写入 trade_orders、更新 trade_positions / lots，并影响账户现金账本。",
                "已实现（本地模拟账本）。需先有盘后扫描写入的目标池标的，并开启总闸与本任务。"
        ));
        m.put("sync-orders", new Detail(
                "同步委托/成交状态：将本地未完结订单与券商柜台状态对齐。",
                "针对本地 trade_orders 中待确认/部分成交等状态的订单。",
                "默认 FIXED_RATE 约 10 秒。",
                "更新 trade_orders 状态、成交量/成交价等字段。",
                "未实现：待接入券商委托查询 API；当前仅有本地桩，开启后不会真正对账。"
        ));
        m.put("position-pnl-sync", new Detail(
                "持仓盈亏同步：用最新价估算持仓市值与浮动概况，便于监控。",
                "本地 trade_positions / 网关持仓快照中的持仓标的。",
                "默认交易时段每分钟一次（CRON）。",
                "主要打日志与本地估值；对接券商后可回写成本/可用数量。",
                "未实现完整券商对接：当前用本地持仓 + 最新价估算市值；真实盈亏需成交回报对账。"
        ));
        m.put("settle-after-close", new Detail(
                "收盘后清算：账户日结、权益落库，并做 K 线聚合等收盘批处理。",
                "当日账户与持仓；全市场/池内标的的行情聚合。",
                "默认工作日 15:30（CRON）。",
                "可写入 trade_cashflows、更新聚合元数据；拉取/聚合依赖行情源。",
                "部分能力未实现：账户清算本地可用，真实行情拉取与完整聚合待 API。"
        ));
